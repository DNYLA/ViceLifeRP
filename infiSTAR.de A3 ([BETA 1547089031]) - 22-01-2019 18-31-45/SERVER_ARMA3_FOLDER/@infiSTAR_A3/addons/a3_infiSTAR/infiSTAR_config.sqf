...
_badIDDsToKick = [-1338,-1337,1340,1341,1342,1343,1344,1345,1346,1347,1320,1321,2727,2928,2929,3030,316000,9899,0110];

/* _badIDDsToClose: Forbidden Idds that will get closed by the AH */
_badIDDsToClose =
[
	2,3,7,17,19,25,26,27,28,29,30,31,32,37,40,41,43,44,45,51,52,53,56,59,62,64,69,71,74,85,
	106,110,126,127,132,133,146,147,150,151,152,153,154,155,156,159,162,164,165,166,167,262,
	314,632,2121,148,163,129,169,157,
	312 // Zeus Menu
];
...
[
	...
	'fredtargetkill','loopfredtpyoutome','epochTp','AdminLst','BB_Pr0_Esp','BBProEsp','epochMapMP','CALLRESVR','EPOCH_spawnVehicle_PVS',
	...
	'atext','nd','vspeed','Ug8YtyGyvguGF','inv','rspwn','pList','loldami','T','bowonky','aimbott','Admin_Layout','markeresp','allMrk','MakeRandomSpace',
	...
	'bl4ck1ist','actualunit','mark_player','unitList_vec','yo2','actualunit_vec','typeVec','mark','r_menu','hfghfg','vhnlist','work','Intro',
	...
	'PermDialogSelected','AdminDialogList','pfKeygen','pfScanUnits','pfPickPlayer','pfshnext','pfnlist','pfselecteditem','pfshnmenu',
	...
	'VAR56401668319_secret','myPubVar','XXMMWW_boxquad','Init_Menu_Fury','A3RANDVARrpv1tpv','fnc_nestf','XXMMWW_keybinds','smissles','wooden_velo','vabox'
];
...
if(_isWindows && _armalog)then{
	...
	FN_ARMA_REMOTELOG = {
		...
		"armalog" callExtension ("8"+"a3"+";;"+_logname+";;"+_logentry+";;"+"22-01-2019 18-31-45 - [BETA 1547089031]");
	};
...
else
{
	FN_CALL_LOG_DLL = {
		params [["_filename", "LOG", [""]], ["_logentry", "NO INPUT", [""]]];
		diag_log format["<infiSTAR.de>%1| %2   [22-01-2019 18-31-45 - [BETA 1547089031]]",_filename,_logentry];
	};
	FN_ARMA_FETCHDATA = {};
	FN_ARMA_FIREANDFORGET = {};
	FN_ARMA_REMOTELOG = {};
	...
	[
		"",
		{
			with uiNameSpace do
			{
				...
				_txt = "";
				_txt = _txt + "<t size=""2"" align=""center"" shadowColor=""#131718"" color=""#FFFFFF"">GLOBAL BANNED!</t><br/><br/><br/>";
				_txt = _txt + "<t size=""1.1"" align=""center"" shadowColor=""#131718"" color=""#FFFFFF""><a href=""https://discord.gg/8yUCTVx"">APPEAL A BAN</a></t><br/>";
				_txt = _txt + "<t size="".75"" align=""center"" shadowColor=""#131718"" color=""#FFFFFF"">Only appeal a ban if you did NOT<br/>hack/buguse/glitch/dupe/lagswitch/test a pbo for a friend.</t><br/><br/><br/><br/>";
				_txt = _txt + "<t size="".9"" align=""right"" shadowColor=""#131718"" color=""#FFFFFF"">infiSTAR.de</t><br/>";
				_txt = _txt + "<t size="".85"" align=""right"" shadowColor=""#131718"" color=""#FFFFFF""><a href=""https://www.paypal.me/infiSTAR/10"">click to donate</t><br/>";
				{
					{ _x ctrlEnable false; } forEach (allControls _x);
					ctrlDelete (_x displayCtrl 14);
					_ctrl = _x ctrlCreate ["IGUIBack",14];
					_ctrl ctrlSetPosition [safezoneX, safezoneY, safeZoneW, safezoneH];
					_ctrl ctrlSetBackgroundColor [0,0,0,1];
					_ctrl ctrlCommit 0;
					ctrlSetFocus _ctrl;
					ctrlDelete (_x displayCtrl 15);
					_ctrl = _x ctrlCreate ["RscStructuredText",15];
					_ctrl ctrlSetPosition [safezoneX + (safeZoneW * 0.5) - (safeZoneW * 0.125), safezoneY + (safeZoneH * 0.5) - (safeZoneH * 0.125), safeZoneW * 0.25, safezoneH * 0.25];
					_ctrl ctrlSetBackgroundColor [0,0,0,1];
					_ctrl ctrlCommit 0;
					_ctrl ctrlSetStructuredText parseText _txt;
					ctrlSetFocus _ctrl;
				} forEach [findDisplay 0];
				[] spawn {for '_i' from 0 to 100 do {findDisplay 46 closeDisplay 0;};};
			};
		}
	] remoteExecCall ["call",_this,false];
};
/* ********************************************************************************* */
// OwnerEmail: danny13653@gmail.com
/* ********************************************************************************* */
/* ********************************************************************************* */
comment "Antihack & AdminTools - Christian Lorenzen - www.infiSTAR.de - License: (CC)";
diag_log format["<infiSTAR.de> %1 - including AntiHack",time];
#include "A3AH.sqf"
diag_log format["<infiSTAR.de> %1 - AntiHack included!",time];
diag_log format["<infiSTAR.de> %1 - including AdminTools",time];
#include "A3AT.sqf"
diag_log format["<infiSTAR.de> %1 - AdminTools included!",time];