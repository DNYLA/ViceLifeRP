...
/* *********************************www.infiSTAR.de********************************* */
diag_log '<infiSTAR.de> VERSION: infiSTAR.de 14-01-2019 00-41-21 (v259)';
comment 'Antihack & AdminTools - Christian Lorenzen - www.infiSTAR.de - License: (CC)';
UPDATEEMAIL='danny13653@gmail.com';
INFISTARVERSION='14-01-2019 00-41-21 - v259';
...
if(!_return)exitWith
{
	diag_log format['<infiSTAR.de> %1 - serverCommandPassword NOT SET!    danny13653@gmail.com - 14-01-2019 00-41-21 - v259 - %2 - %3',time,serverName,productVersion];
...
{
	diag_log format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp is not in your MPMission or you forgot to #include it within your description.ext   [danny13653@gmail.com - 14-01-2019 00-41-21 - v259 - %2 - %3]',_t,serverName,productVersion];
};
_test = [0,{}] execFSM 'call.fsm';
_test = [0,{}] execFSM 'call.fsm';
if(_test isEqualTo 0)exitWith
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!    danny13653@gmail.com - 14-01-2019 00-41-21 - v259 - %2 - %3',_t,serverName,productVersion];
...
fn_infiSTAR_ObjName = compileFinal "if(alive _this)then{_n = name _this;_this setVariable['playerName',_n];_n}else{_n = _this getVariable['playerName',''];if(_n isEqualTo '')then{if!(getPlayerUID _this isEqualTo '')then{_n = name _this;_this setVariable['playerName',_n];};};_n}";
publicVariable "fn_infiSTAR_ObjName";
/****************************************************************************************************/
/****************************************************************************************************/
/****************************************************************************************************/
FNC_A3_CUSTOMLOG = compileFinal "
	params['_logname','_logentry'];
	diag_log format['<infiSTAR.de>%1| %2   [14-01-2019 00-41-21 - v259]',_logname,_logentry];
...
_massMessage = call _fnc_RandomGen;['RANDOMVAR',format['_massMessage: %1',_massMessage]] call FNC_A3_CUSTOMLOG;
_massSysMessage = call _fnc_RandomGen;['RANDOMVAR',format['_massSysMessage: %1',_massSysMessage]] call FNC_A3_CUSTOMLOG;
...
_checkidicheckcheck = call _fnc_RandomGen;['RANDOMVAR',format['_checkidicheckcheck: %1',_checkidicheckcheck]] call FNC_A3_CUSTOMLOG;
_lastshotmade = call _fnc_RandomGen;['RANDOMVAR',format['_lastshotmade: %1',_lastshotmade]] call FNC_A3_CUSTOMLOG;
_dellocveh = call _fnc_RandomGen;['RANDOMVAR',format['_dellocveh: %1',_dellocveh]] call FNC_A3_CUSTOMLOG;
...
_MAKE_VAR_DUMP_RANDOM = call _fnc_RandomGen;['RANDOMVAR',format['_MAKE_VAR_DUMP_RANDOM: %1',_MAKE_VAR_DUMP_RANDOM]] call FNC_A3_CUSTOMLOG;
_MAKE_VAR_DUMP_CLIENT = call _fnc_RandomGen;['RANDOMVAR',format['_MAKE_VAR_DUMP_CLIENT: %1',_MAKE_VAR_DUMP_CLIENT]] call FNC_A3_CUSTOMLOG;
_DUMPINGPLAYERUIDARRAY = call _fnc_RandomGen;['RANDOMVAR',format['_DUMPINGPLAYERUIDARRAY: %1',_DUMPINGPLAYERUIDARRAY]] call FNC_A3_CUSTOMLOG;
_CURRENTLYDUMPING = call _fnc_RandomGen;['RANDOMVAR',format['_CURRENTLYDUMPING: %1',_CURRENTLYDUMPING]] call FNC_A3_CUSTOMLOG;
_antiantihack_rndvar = call _fnc_RandomGen;['RANDOMVAR',format['_antiantihack_rndvar: %1',_antiantihack_rndvar]] call FNC_A3_CUSTOMLOG;
...
fn_infiSTAR_checkGlobalBanState = {
	...
	"; if(_USE_GLOBAL_BAN_CHECK)then{ _A3AHstring = _A3AHstring + "
		"; if(_BAN_GLOBAL_BANNED_LOCALLY)then{ _A3AHstring = _A3AHstring + "
			[_name,_uid,format['GLOBAL BAN: %1',_res]] call FNC_A3_infiSTARBAN;
		"; }else{ _A3AHstring = _A3AHstring + "
			[_name,_uid,_owner,format['GLOBAL BAN: %1',_res]] call FNC_INFISERVERKICK;
		"; }; _A3AHstring = _A3AHstring + "
	"; }else{ _A3AHstring = _A3AHstring + "
		_log = format['%1(%2) | GLOBAL BANNED PLAYER (NOT BANNED LOCALLY)',_name,_uid];
	...
	if(_what in ['BAN','TMPBAN','HLOG','HLOG_SKICK'])exitWith
	{
		...
		if(_what in ['BAN','TMPBAN'])exitWith
		{
			...
			else
			{
				...
				{
					[_name,_puid,_work] call FNC_A3_infiSTARTMPBAN;
				};
			};
		};
	};
...
{
	_mytime = call fnc_getserverTime;
	_log = _mytime + 'AdminReq tokenreceived is empty (v259)';
	_log call FNC_A3_SURVEILLANCELOG;
};
if(count _array < 2)exitWith
{
	_mytime = call fnc_getserverTime;
	_log = _mytime + format['AdminReq bad array: %1 (v259)',_array];
	_log call FNC_A3_SURVEILLANCELOG;
};
if(_clientNetID isEqualTo '')exitWith
{
	_mytime = call fnc_getserverTime;
	_log = _mytime + 'AdminReq clientNetID is empty (v259)';
	_log call FNC_A3_SURVEILLANCELOG;
};
...
{
	_mytime = call fnc_getserverTime;
	_log = _mytime + 'AdminReq _ObjFromNetID is Null (v259)';
	_log call FNC_A3_SURVEILLANCELOG;
};
...
{
	_log = format['%1(%2) | AdminReq - Bad PUID / Token: Token received [%3] belongs to [%4] and not [%2] (v259)',_clientName,_clientUID,_tokenreceived,_uid_by_token];
	_log call FNC_A3_SURVEILLANCELOG;
};
...
{
	_mytime = call fnc_getserverTime;
	_log = _mytime + format['AdminReq _playerObj != _ObjFromNetID - %1/2  (v259)',_playerObj,_ObjFromNetID];
	_log call FNC_A3_SURVEILLANCELOG;
...
_FNC_AH_KICKLOGSPAWN =
{
	...
	if(isNil '_input')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + 'FNC_AH_KICKLOGSPAWN _this is Nil (v259)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	if(isNil '_tokenreceived')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + '_tokenreceived _this is Nil (v259)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if(typeName _tokenreceived != 'STRING')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['_tokenreceived wrong type %1 (v259)',typeName _tokenreceived];
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	if(isNil '_arraysent')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + '_arraysent _this is Nil (v259)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if(typeName _arraysent != 'ARRAY')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['_arraysent wrong type %1 (v259)',typeName _arraysent];
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	if(typeName _netId != 'STRING')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['_netId wrong type %1 (v259)',typeName _netId];
		_log call FNC_A3_SURVEILLANCELOG;
	};
...
fn_adminConnected_infiSTAR = {
	...
	if(isNil'infiSTAR_MAIN_CODE')exitWith
	{
		...
		diag_log '<infiSTAR.de> ERROR - infiSTAR_MAIN_CODE is NIL! (v259)';
	};
	...
	"; if(_LogAdminActions)then{ _A3AHstring = _A3AHstring + "
		_fnc_adminLog = if(_uid in "+str _devs+")then{{}}else{
			diag_log format['<infiSTAR.de> ******ADMIN-LOGIN******: %1(%2) (v259)',_name,_uid];
			compile(format['[''%1'',''%2'',''ALOG'',toArray _this] call "+_AHKickLog+"',_name,_uid])
		};
	"; }; _A3AHstring = _A3AHstring + "
	
	[
		[_uid, _fnc_adminLog, _admin, _isNormal, infiSTAR_MAIN_CODE],
		{
			params [
				['_uid','',['']],
				['_fnc_adminLog',{},[{}]],
...
fn_playerConnected_infiSTAR = {
	...
	else
	{
		...
		_url = _url - [35];
		_url = toString _url;
		_url = 'http://load.infistar.de/rp.php?mod=A3&steamid=' + _url;
		_code = {
			params ['_uid','_name','_url'];
	...
	if(!isNil'_actionParams')then
	{
		...
		[_name,_puid,_AHKickOFF,_AHKickLog,_admins] spawn {
			...
			while{1==1}do
			{
				...
				} forEach [-1338,-1337,17,19,30,32,45,56,59,62,64,69,71,110,132,155,156,162,165,166,167,1320,1321,2727,2928,2929,3030,316000,9899,0110];
...
"; }; _A3AHstring = _A3AHstring + "
	...
	_devLog = format['<infiSTAR.de> %1 VERSION: infiSTAR.de 14-01-2019 00-41-21 (v259) - server running: %2:%3:%4',VERSIONCHECKRESULT,_hours,_minutes,_seconds];diag_log _devLog;
...
_DO_THIS_MORE_OFTEN = {
	...
	[] spawn {scriptName format['MORE_OFTEN_%1',time];uiSleep 1;if(isNil '""+_antiantihack_rndvar+""')then{diag_log '<infiSTAR.de> kicked to lobby #9';(findDisplay 46)closeDisplay 0;};};
...
['',_zeroCode,-2,'zerocodeJIP'] call FN_infiSTAR_S;
";
